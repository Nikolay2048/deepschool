...
### 2026-07-28 — autograd и training loop

- Разобраны leaf tensors, `requires_grad`, `.grad` и `.grad_fn`.
- Обучена линейная модель `y = 2x + 1` вручную средствами autograd.
- Получены параметры `w = 2.0016`, `b = 0.9967`.
- Экспериментально показаны накопление градиентов и расходимость при слишком большом learning rate.
